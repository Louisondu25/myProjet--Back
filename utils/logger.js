...
const logStream = fs.createWriteStream(`logs/${new Date().toLocaleString().split('/').join(' - ').split(':').join('_')}_${process.env.npm_lifecycle_event !== 'test' ? 'PRODUCTION' : 'TESTING'}.log`, { flags: 'a' });
